...
namespace Datadog.Trace.Tests
{
    public class AgentWriterTests
    {
        ...
        public async Task WriteTrace_2Traces_SendToApi()
        {
            ...
            var trace = new[] { new AbstractSpan(_spanContext, start: null) };
            _agentWriter.WriteTrace(trace);
            await Task.Delay(TimeSpan.FromSeconds(2));
            _api.Verify(x => x.SendTracesAsync(It.Is<AbstractSpan[][]>(y => y.Single().Equals(trace))), Times.Once);

            trace = new[] { new AbstractSpan(_spanContext, start: null) };
            _agentWriter.WriteTrace(trace);
            await Task.Delay(TimeSpan.FromSeconds(2));
            _api.Verify(x => x.SendTracesAsync(It.Is<AbstractSpan[][]>(y => y.Single().Equals(trace))), Times.Once);
        }
